function handleIncomingCall(data) {
    CALL_ROLE = 'callee';
    window.CALL_CHAT_ID = data.chatId || currentChatId;
    ...
    if (CALL_ROLE === 'caller') {
        if (data && data.action === 'accept') {
            ...
            try { 
                ...
                RTC_LOCAL.getTracks().forEach(t => {
                    const senders = RTC_PC.getSenders();
                    if (!senders.find(s => s.track === t)) RTC_PC.addTrack(t, RTC_LOCAL);
                });
                const offerOptions = { offerToReceiveAudio: true };
                if (CALL_MEDIA === 'video') offerOptions.offerToReceiveVideo = true;
                const offer = await RTC_PC.createOffer(offerOptions);
                await RTC_PC.setLocalDescription(offer); 
                const msg = { type: 'webrtc', action: 'offer', chatId: (window.CALL_CHAT_ID || currentChatId), senderId: myUserId, sdp: RTC_PC.localDescription }; 
...
(function () {
    if (typeof callAnswer !== 'undefined' && callAnswer && !callAnswer.__bound) {
        callAnswer.addEventListener('click', function () {
            ...
            try { const ring = document.getElementById('ringtone'); if (ring) { ring.pause(); ring.currentTime = 0; } } catch(e){}
            const resp = { type: 'call_response', action: 'accept', chatId: (window.CALL_CHAT_ID || currentChatId), senderId: myUserId };
            try {
    ...
    if (typeof callDecline !== 'undefined' && callDecline && !callDecline.__bound) {
        callDecline.addEventListener('click', function () {
            ...
            if (alreadyAccepted) {
                ...
                try {
                    const resp = { type: 'call_response', action: 'decline', chatId: (window.CALL_CHAT_ID || currentChatId), senderId: myUserId };
                    if (typeof sendSocketMessage === 'function') sendSocketMessage(resp);
                    else if (typeof sendMessageViaSocket === 'function') sendMessageViaSocket(resp);
                } catch (e) { }
                __hangup(false);
...
let CALL_ROLE = null;
let CALL_MEDIA = null;
let CALL_PEER_NAME = null;


/**
 * Инициализация WebRTC соединения (PeerConnection).
 * Здесь мы указываем STUN сервер Google для обхода NAT и связываем все события.
 * Это сердце наших аудио- и видеозвонков!
 */
function __createPC() {
    RTC_PC = new RTCPeerConnection({ iceServers: [{ urls: 'stun:stun.l.google.com:19302' }] });
    RTC_PC.onicecandidate = (ev) => {
        if (ev.candidate) {
            const msg = { type: 'webrtc', action: 'ice', chatId: (window.CALL_CHAT_ID || currentChatId), senderId: myUserId, candidate: ev.candidate };
...
/**
 * Запрашиваем доступ к камере и микрофону у пользователя.
 * Важно: браузеры (особенно мобильные) требуют, чтобы этот метод вызывался
 * только в ответ на прямой клик пользователя! Иначе будет ошибка безопасности.
 */
...
/**
 * Обработчик всех входящих сигналов WebRTC (offer, answer, ice-кандидаты).
 * Отвечает за процесс "рукопожатия" (handshake) между двумя устройствами.
 */
async function handleWebRTCSignal(data) {
    try {
        ...
        if (data.action === 'offer') {
            ...
            if (!RTC_LOCAL) { RTC_LOCAL = await __getMedia(CALL_MEDIA || 'audio'); }
            RTC_LOCAL.getTracks().forEach(t => {
                const senders = RTC_PC.getSenders();
                if (!senders.find(s => s.track === t)) RTC_PC.addTrack(t, RTC_LOCAL);
            });
            ...
            const resp = { type: 'webrtc', action: 'answer', chatId: (window.CALL_CHAT_ID || currentChatId), senderId: myUserId, sdp: RTC_PC.localDescription };
...
function startCallToContact(contactId) {
    const contact = contactsData.find(c => c.id === contactId);
    if (!contact) return;

    callName.textContent = contact.name;
    callAvatar.innerHTML = `<img src="${contact.avatar}" alt="${contact.name}">`;
    callStatus.textContent = t('calling');
    callContainer.classList.add('active');

    CALL_ROLE = 'caller';
    CALL_MEDIA = (type === 'video') ? 'video' : 'audio';
    if (callAnswer) callAnswer.style.display = 'none';
    if (callDecline) callDecline.style.display = '';
    try {
        const payload = { type: 'call', action: 'invite', media: CALL_MEDIA, chatId: (window.CALL_CHAT_ID || currentChatId), senderId: myUserId, fromName: (chat && chat.name) ? chat.name : undefined, fromAvatar: (chat && chat.avatar) ? chat.avatar : undefined };
        if (typeof sendSocketMessage === 'function') { sendSocketMessage(payload); }
        else if (typeof sendMessageViaSocket === 'function') { sendMessageViaSocket(payload); }
    } catch (e) { console.warn('call invite send failed', e); }
...
function startCallToContactByName(name) {
    callName.textContent = name;
    callAvatar.innerHTML = `<i class="fas fa-user"></i>`;
    callStatus.textContent = t('calling');
    callContainer.classList.add('active');
...
async function startCall(type) {
    ...
    window.CALL_CHAT_ID = currentChatId;

    try {
        if (!RTC_LOCAL) {
            RTC_LOCAL = await __getMedia(type);
        }
    } catch (e) {
...
(c => c.id === currentChatId);
    ...
    callAvatar.innerHTML = `<img src="${chat.avatar}" alt="${chat.name}">`;
    callStatus.textContent = type === 'audio' ? t('calling') : t('videoCalling');
    callContainer.classList.add('active');
    ...
    try {
        ...
        if (typeof sendSocketMessage === 'function') {
            const payload = { type: 'call', action: 'invite', media: (type === 'video' ? 'video' : 'audio'), chatId: (window.CALL_CHAT_ID || currentChatId), senderId: myUserId, fromName: (chat && chat.name) ? chat.name : undefined, fromAvatar: (chat && chat.avatar) ? chat.avatar : undefined };
...
function __startRingTimeout(ms) {
    __clearRingTimeout();
    const delay = (typeof ms === 'number' && ms > 0) ? ms : 30000;
    __RING_TIMEOUT_ID = setTimeout(() => {
        if (CALL_SESSION_ACTIVE) return;
        if (callStatus) callStatus.textContent = t('noAnswer');

        setTimeout(() => { try { __hangup(true); } catch (e) { } }, 1500);
    }, delay);
}
function __clearRingTimeout() {
    if (__RING_TIMEOUT_ID) { clearTimeout(__RING_TIMEOUT_ID); __RING_TIMEOUT_ID = null; }
}
